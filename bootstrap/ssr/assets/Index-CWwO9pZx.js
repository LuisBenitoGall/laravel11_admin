...
import { M as ModalUserCreate } from "./ModalUserCreate-svcrFzNc.js";
...
function ModalMarketingListFromContacts({ show, onClose, filters = {} }) {
  ...
  const handleConfirm = () => {
    ...
    router.post(
      route("marketing-lists.store-from-contacts"),
      {
        ...
        redirect_filters: filters || {}
        // 👈 aquí viajan los filtros
      },
      {
        preserveScroll: true,
        onSuccess: () => {
...
function Index({
  ...
  const totalContacts = ((_a = contacts == null ? void 0 : contacts.meta) == null ? void 0 : _a.total) ?? 0;
  ...
  const handleToggleSelectAll = async () => {
    ...
    const currentPageIds = ((contacts == null ? void 0 : contacts.data) || []).map((c) => c.id);
    setSelectedContactIds((prev) => {
    ...
    try {
      ...
      const rows = await filteredData(tableQueryParams);
      const allIds = rows.map((row) => row.id).filter((id) => id !== null && id !== void 0);
      setSelectedContactIds((prev) => {
