...
export default function Index({ auth, session, title, subtitle, accounts, queryParams: rawQueryParams = {}, availableLocales }) {
    ...
    const columns = [
        ...
        { key: 'duration', label: (<> {__('duracion')} <small>({__('dias')})</small> </>), sort: false, class_th: 'text-center', class_td: 'text-end'}
    ];
            ...
            <div className="contents">
                <div className="row">
                    <div className="controls d-flex align-items-center">
                        ...
                        <TableExporter filename={ __('cuentas') } columns={columns} fetchData={filteredData}/>
                    </div>
                </div>

                {/* Tabla */}
                <div className="table-responsive">
                    <Table className="table table-nowrap table-striped align-middle mb-0" id="tblAccounts">
                        <thead>
                            <tr>
                                {columns.map(col => (
                                    ...
                                    <td className="text-end">
                                        {permissions?.['accounts.edit'] && (
                                            <OverlayTrigger
                                                ...
                                                placement="top"
                                                overlay={<Tooltip className="ttp-top">{ account.status == 1 ? __('cuenta_activa') : __('cuenta_inactiva') }</Tooltip>}
                                            >
                                                <StatusButton 
