# crm-contacts-import-account-resolution

## Purpose

Reglas de resolución de `CrmAccount` durante la importación Excel de contactos CRM: prioridad por `tax_id` (NIF), segundo criterio por `normalized_name` (`Str::slug`), y creación solo si no hay match, siempre acotado al `company_id` de sesión.
